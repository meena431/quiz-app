...
import { resultStyles } from '../assets/styles';
import axios from 'axios'
import { toast } from 'react-toastify'

// Show batch according to percentage
const Badge = ({ percent }) => {
  if (percent >= 85)
...
const Result = ({ apiBase = "http://localhost:4000" }) => {
  ...
  // Fetch results when component mounts or when selectedTechnology changes
  useEffect(() => {
    let mounted = true;
  ...
  // Fetch all results once (or when apiBase changes) to built a list of available 'technologies' for filter buttons.
  useEffect(() => {
    let mounted = true;
  ...
  // 'Summary' is memoized so it only recalculates when 'result' changes.
